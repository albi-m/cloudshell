...
/// key with the given value, waits for the stream to resolve, then returns
/// the container.
Future<ProviderContainer> _containerWithSetting(
    String key, String? value) async {
  final container = ProviderContainer(overrides: [
    settingProvider(key).overrideWith((ref) => Stream.value(value)),
  ]);
  // Wait for the stream to emit so the computed providers see `data`.
  await container.read(settingProvider(key).future);
  return container;
}
...
void main() {
  group('themeModeProvider', () {
    test('defaults to dark when no setting', () async {
      final container = await _containerWithSetting('theme_mode', null);
      addTearDown(container.dispose);
      expect(container.read(themeModeProvider), ThemeMode.dark);
    });

    test('returns light when setting is light', () async {
      final container = await _containerWithSetting('theme_mode', 'light');
      addTearDown(container.dispose);
      expect(container.read(themeModeProvider), ThemeMode.light);
    });

    test('returns dark when setting is dark', () async {
      final container = await _containerWithSetting('theme_mode', 'dark');
      addTearDown(container.dispose);
      expect(container.read(themeModeProvider), ThemeMode.dark);
    });

    test('returns dark for unknown values', () async {
      final container = await _containerWithSetting('theme_mode', 'garbage');
      addTearDown(container.dispose);
      expect(container.read(themeModeProvider), ThemeMode.dark);
    });
  });
  ...
  group('terminalThemeIdProvider', () {
    test('defaults to cloudshell_default when null', () async {
      final container =
          await _containerWithSetting('terminal_theme', null);
      addTearDown(container.dispose);
    ...
    test('returns stored theme ID', () async {
      final container =
          await _containerWithSetting('terminal_theme', 'dracula');
      addTearDown(container.dispose);
  ...
  group('terminalCursorStyleProvider', () {
    test('defaults to block when null', () async {
      final container =
          await _containerWithSetting('terminal_cursor_style', null);
      addTearDown(container.dispose);
    ...
    test('returns stored cursor style', () async {
      final container =
          await _containerWithSetting('terminal_cursor_style', 'underline');
      addTearDown(container.dispose);
    ...
    test('returns bar style', () async {
      final container =
          await _containerWithSetting('terminal_cursor_style', 'bar');
      addTearDown(container.dispose);
  ...
  group('onboardingCompleteProvider', () {
    test('returns false when setting is false', () async {
      final container =
          await _containerWithSetting('onboarding_complete', 'false');
      addTearDown(container.dispose);
    ...
    test('returns true when setting is true', () async {
      final container =
          await _containerWithSetting('onboarding_complete', 'true');
      addTearDown(container.dispose);
    ...
    test('returns false when setting is null', () async {
      final container =
          await _containerWithSetting('onboarding_complete', null);
      addTearDown(container.dispose);
      expect(container.read(onboardingCompleteProvider), isFalse);
    });
  });
  ...
  group('localeProvider', () {
    test('returns null for system', () async {
      final container = await _containerWithSetting('language', 'system');
      addTearDown(container.dispose);
      expect(container.read(localeProvider), isNull);
    });

    test('returns null for empty string', () async {
      final container = await _containerWithSetting('language', '');
      addTearDown(container.dispose);
      expect(container.read(localeProvider), isNull);
    });

    test('returns null for null', () async {
      final container = await _containerWithSetting('language', null);
      addTearDown(container.dispose);
      expect(container.read(localeProvider), isNull);
    });

    test('returns Locale for valid language code', () async {
      final container = await _containerWithSetting('language', 'ja');
      addTearDown(container.dispose);
      expect(container.read(localeProvider), const Locale('ja'));
    });

    test('returns Locale for Spanish', () async {
      final container = await _containerWithSetting('language', 'es');
      addTearDown(container.dispose);
  ...
  group('terminalLigaturesProvider', () {
    test('returns false when null (only true for explicit "true")', () async {
      final container =
          await _containerWithSetting('terminal_ligatures', null);
      addTearDown(container.dispose);
    ...
    test('returns false when setting is false', () async {
      final container =
          await _containerWithSetting('terminal_ligatures', 'false');
      addTearDown(container.dispose);
      expect(container.read(terminalLigaturesProvider), isFalse);
    });
    ...
    test('returns true when setting is true', () async {
      final container =
          await _containerWithSetting('terminal_ligatures', 'true');
      addTearDown(container.dispose);
  ...
  group('commandNotifyEnabledProvider', () {
    test('defaults to true when null', () async {
      final container =
          await _containerWithSetting('command_notify_enabled', null);
      addTearDown(container.dispose);
    ...
    test('returns false when setting is false', () async {
      final container =
          await _containerWithSetting('command_notify_enabled', 'false');
      addTearDown(container.dispose);
    ...
    test('returns true when setting is true', () async {
      final container =
          await _containerWithSetting('command_notify_enabled', 'true');
      addTearDown(container.dispose);
      expect(container.read(commandNotifyEnabledProvider), isTrue);
    });
  });
